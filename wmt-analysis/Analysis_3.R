# Run the analysis with participants who has very standard training dosage and further excluded those who potentially didn't follow the instruction according to the pre-registered exclusion criteria
...
# sink
save <- paste0(Sys.Date(),"-",format(Sys.time(), "%H"),"-",format(Sys.time(), "%M"),'-Resutls.txt')
sink(paste(analysedResults,save,sep='/'))
...
pre_posthoc$yuendenpent
pre_posthoc$depeffects <- dep.effect(ext_pre$score,ext_post$score)
pre_posthoc$depeffects 
pre_posthoc$prevspostinexp <- ttestBF(x = ext_pre$score,y=ext_post$score, paired=TRUE)
pre_posthoc$prevspostinexp 
...
pre_posthoc$yuenatpost <- yuen(score~group, data = preOrt %>% subset(time == "precision_ort_post"))
pre_posthoc$yuenatpost 
pre_posthoc$yuenatpostEffect <- yuen.effect.ci(score~group, data = preOrt %>% subset(time == "precision_ort_post"))
pre_posthoc$yuenatpostEffect
pre_posthoc$yuenatpostttest <- ttestBF(formula = score~group, data = preOrt %>% subset(time == "precision_ort_post"))
pre_posthoc$yuenatpostttest
print('Post hoc 1 overview:')
pre_posthoc
print('Post hoc 1 END')
...
mrt_posthoc <- c()
ext_pre_mrt <- mrtVS %>% subset(group == "experiment" & time == "mrtPre")
ext_post_mrt <- mrtVS %>% subset(group == "experiment" & time == "mrtPost")
ctl_pre_mrt <- mrtVS %>% subset(group == "control" & time == "mrtPre")
ctl_post_mrt <- mrtVS %>% subset(group == "control" & time == "mrtPost")

yuend(ext_pre_mrt$score,ext_post_mrt$score)
dep.effect(ext_pre_mrt$score,ext_post_mrt$score)
ttestBF(x = ext_pre$score,y=ext_post$score, paired=TRUE)

mrt_posthoc$yuendenpent <- yuend(ctl_pre$score,ctl_post$score)
mrt_posthoc$yuendenpent
mrt_posthoc$depeffects <- dep.effect(ctl_pre$score,ctl_post$score)
mrt_posthoc$depeffects 
mrt_posthoc$prevspostinctrl <- ttestBF(x = ctl_pre$score,y=ctl_post$score, paired=TRUE)
mrt_posthoc$prevspostinctrl 

mrt_posthoc$yuenatpost <- yuen(score~group, data = mrtVS %>% subset(time == "mrtPost"))
mrt_posthoc$yuenatpost 
mrt_posthoc$yuenatpostEffect <- yuen.effect.ci(score~group, data = mrtVS %>% subset(time == "mrtPost"))
mrt_posthoc$yuenatpostEffect
mrt_posthoc$yuenatpostttest <- ttestBF(formula = score~group, data = mrtVS %>% subset(time == "mrtPost"))
mrt_posthoc$yuenatpostttest
print('Post hoc 2 overview:')
mrt_posthoc
print('Post hoc 2 END')
...
print('Exploratory result-START')
...
chisq2
b2 <- contingencyTableBF(contingencyPostTB,sampleType = 'poisson')
b2
print('Exploratory result-END')
